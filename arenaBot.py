...
async def get_leaderboard(interaction: discord.Interaction):
    ...
    for user_raw in leaderboard_raw:
        user = interaction.guild.get_member(int(user_raw["discord_id"]))
        ...
        if mode == "HONOR":
            ...
            embed.add_field(name="value" , value=user_raw["games_data"]["games_won"])
        else:
            embed.add_field(name="value" , value=user_raw["games_data"]["games_played"])
